...
function renderPlatformOperations() {
  if (state.user?.role !== "PLATFORM_ADMIN") {
    $("#viewContent").innerHTML = `<section class="admin-board"><p>플랫폼 관리자 권한이 필요합니다.</p></section>`;
    return;
  }
  ...
  $("#viewContent").innerHTML = `<section class="platform-ops-layout">
    <section class="admin-board platform-center-list">
      <div class="section-heading">
        <div><span>중앙 운영</span><h3>체육관 중앙 관제</h3></div>
        <button id="togglePlatformCenterCreate">${state.showPlatformCenterCreateForm ? "닫기" : "체육관 추가"}</button>
      </div>
      <p class="form-message">${escapeHtml(state.platformMessage || "체육관 상태와 계약 변경은 다음 인증 요청부터 적용됩니다.")}</p>
      ${state.showPlatformCenterCreateForm ? platformCenterCreateForm() : ""}
      <label class="platform-search"><span>검색</span><input id="platformCenterSearch" value="${escapeHtml(state.platformCenterSearch)}" placeholder="체육관명, 코드, 요금제" /></label>
      <div class="table-scroll"><table class="admin-table platform-center-table">
        <thead><tr><th>체육관</th><th>운영 상태</th><th>이용 계약</th><th>등록 인원</th><th>최근 활동</th></tr></thead>
        <tbody>${centers.map(platformCenterRow).join("") || `<tr><td colspan="5">표시할 체육관이 없습니다.</td></tr>`}</tbody>
      </table></div>
    </section>
    ${selected ? platformCenterDetail(selected) : `<section class="admin-board"><p>먼저 체육관을 생성하세요.</p></section>`}
    <section class="admin-board platform-audit-panel">
      <div class="section-heading"><div><span>변경 이력</span><h3>관리자 작업 기록</h3></div></div>
...
function platformCenterRow(center) {
  ...
  return `<tr class="platform-center-row ${selected ? "selected" : ""}" data-platform-center="${center.id}">
    ...
    <td>${people}명<small class="table-subline">관장 ${Number(center.owner_count || 0)} · 코치 ${Number(center.coach_count || 0)} · 회원 ${Number(center.member_count || 0)}</small></td>
    <td><span>접속 ${formatPlatformDate(center.last_login_at)}</span><small class="table-subline">운동 ${formatPlatformDate(center.last_session_at)}</small></td>
  </tr>`;
}
...
function platformCenterDetail(center) {
  return `<section class="admin-board platform-center-detail">
    <div class="section-heading">
      <div><span>체육관 상세 정보</span><h3>${escapeHtml(center.name)}</h3></div>
    ...
    <form id="platformCenterForm" class="center-form-grid compact">
      <label class="center-field"><span>체육관명</span><input name="name" value="${escapeHtml(center.name)}" required /></label>
      <label class="center-field"><span>체육관 코드</span><input name="code" value="${escapeHtml(center.code)}" required /></label>
      <label class="center-field"><span>운영 상태</span><select name="status">${optionsFor(["ACTIVE", "SUSPENDED", "CLOSED"], center.status)}</select></label>
      <div class="center-actions"><button>센터 정보 저장</button></div>
    </form>
    <form id="platformSubscriptionForm" class="center-form-grid compact platform-contract-form">
      <label class="center-field"><span>요금제</span><input name="plan_code" value="${escapeHtml(center.plan_code || "starter")}" required /></label>
      <label class="center-field"><span>계약 상태</span><select name="status">${optionsFor(["TRIAL", "ACTIVE", "PAST_DUE", "SUSPENDED", "CANCELED", "EXPIRED"], center.subscription_status)}</select></label>
      <label class="center-field"><span>시작일</span><input name="starts_at" type="date" value="${escapeHtml(center.starts_at || "")}" required /></label>
      <label class="center-field"><span>종료일</span><input name="ends_at" type="date" value="${escapeHtml(center.ends_at || "")}" /></label>
      <label class="center-field"><span>최대 회원 수</span><input name="max_members" type="number" min="1" value="${escapeHtml(center.max_members || "")}" /></label>
      <div class="center-actions"><button>계약 정보 저장</button></div>
    </form>
    ${platformFeatureFlags(center)}
    <p class="platform-owner-hint">관장 지정과 계정 생성은 ‘계정 권한’ 메뉴에서 이 체육관을 선택해 진행합니다.</p>
  </section>`;
}
  ...
  return `<section class="platform-feature-panel">
    ...
    <div class="platform-feature-list">${PLATFORM_FEATURE_PRESETS.map(([key, label]) => {
      ...
      return `<form class="platform-feature-row" data-feature-key="${key}">
        <label><input name="enabled" type="checkbox" ${flag.enabled ? "checked" : ""} /><span><strong>${escapeHtml(label)}</strong><small>관리 코드: ${escapeHtml(key)}</small></span></label>
        <select name="rollout_channel">${optionsFor(["STABLE", "BETA"], flag.rollout_channel)}</select>
        <button class="ghost small-button">저장</button>
      </form>`;
    }).join("")}</div>
  </section>`;
}
...
function platformCenterCreateForm() {
  return `<form id="platformCenterCreateForm" class="member-create-form platform-create-form">
    <input name="name" placeholder="체육관명" required />
    <input name="code" placeholder="체육관 코드" pattern="[a-z0-9_-]{3,24}" required />
    <input name="plan_code" value="starter" placeholder="요금제" required />
...
function platformAuditTable(centerId) {
  ...
  return `<div class="table-scroll"><table class="admin-table platform-audit-table">
    <thead><tr><th>처리 시간</th><th>작업 내용</th><th>대상 항목</th><th>처리자</th></tr></thead>
    ...
    </tr>`).join("") || `<tr><td colspan="4">감사 기록이 없습니다.</td></tr>`}</tbody>
  </table></div>`;
}
...
async function createPlatformCenter(event) {
  ...
  try {
    ...
    state.platformMessage = "체육관을 생성했습니다. 계정 권한 메뉴에서 관장을 지정하세요.";
...
async function savePlatformCenter(event, center) {
  ...
  if (body.status !== "ACTIVE" && body.status !== center.status) {
    const confirmed = window.confirm(`${center.name}의 운영 상태를 '${platformValueLabel(body.status)}'(으)로 변경할까요? 소속 사용자의 다음 요청부터 접근이 제한될 수 있습니다.`);
    if (!confirmed) return;
  }
...
async function savePlatformSubscription(event, center) {
  ...
  if (!["TRIAL", "ACTIVE"].includes(body.status) && body.status !== center.subscription_status) {
    const confirmed = window.confirm(`${center.name}의 계약 상태를 '${platformValueLabel(body.status)}'(으)로 변경할까요?`);
    if (!confirmed) return;
  }
  await runPlatformMutation(
...
function platformValueLabel(value) {
  if (!value) return "-";
  return PLATFORM_VALUE_LABELS[value] || value;
}

function platformPlanLabel(value) {
  if (!value) return "-";
  return PLATFORM_PLAN_LABELS[String(value).toLowerCase()] || value;
}

function platformTargetLabel(value) {
  return PLATFORM_FEATURE_PRESETS.find(([key]) => key === value)?.[1] || value || "-";
}
