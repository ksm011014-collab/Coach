...
alter table public.operation_pass_history add column author_name text not null default '';
alter table public.operation_adjustments add column author_name text not null default '';
alter table public.operation_notes add column author_name text not null default '';

create function public.operations_text(value jsonb, field_name text, maximum integer default 500)
returns text language plpgsql immutable set search_path=public as $$
declare result text;
begin
  if jsonb_typeof(value) is distinct from 'string' then raise exception '%: text required',field_name using errcode='22023'; end if;
  result:=btrim(value#>>'{}');
  if length(result)<1 or length(result)>maximum then raise exception '%: invalid length',field_name using errcode='22023'; end if;
  return result;
end $$;
create function public.operations_day(value text)
returns date language plpgsql immutable set search_path=public as $$
declare result date;
begin
  if value is null or value !~ '^\d{4}-\d{2}-\d{2}$' then raise exception 'invalid date' using errcode='22023'; end if;
  result:=value::date;
  if to_char(result,'YYYY-MM-DD')<>value then raise exception 'invalid date' using errcode='22023'; end if;
  return result;
end $$;
create function public.operations_integer(value jsonb, minimum bigint default 0, maximum bigint default 9007199254740991)
returns bigint language plpgsql immutable set search_path=public as $$
declare result bigint;
begin
  if jsonb_typeof(value) not in ('number','string') or value is null or (value#>>'{}') !~ '^\d+$' then raise exception 'integer required' using errcode='22023'; end if;
  result:=(value#>>'{}')::bigint;
  if result<minimum or result>maximum then raise exception 'integer out of range' using errcode='22023'; end if;
  return result;
end $$;
revoke all on function public.operations_text(jsonb,text,integer),public.operations_day(text),public.operations_integer(jsonb,bigint,bigint) from public,anon;
grant execute on function public.operations_text(jsonb,text,integer),public.operations_day(text),public.operations_integer(jsonb,bigint,bigint) to authenticated;

create function public.operations_snapshot()
...
create function public.operations_mutate(p_operation text,p_input jsonb,p_request_id text)
