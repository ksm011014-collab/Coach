...
declare
  ...
  tracking jsonb; gap jsonb; gaps jsonb := '[]'; tracking_total numeric; recorded numeric := 0; gap_end numeric := 0;
begin
  ...
  if report ? 'tracking' then
    tracking := report->'tracking';
    if jsonb_typeof(tracking) is distinct from 'object'
      or jsonb_typeof(tracking->'gaps') is distinct from 'array'
      or jsonb_typeof(tracking->'truncated') is distinct from 'boolean'
      or jsonb_typeof(tracking->'total_ms') is distinct from 'number' then
      raise exception 'invalid tracking evidence' using errcode='22023';
    end if;
    tracking_total := (tracking->>'total_ms')::numeric;
    if jsonb_array_length(tracking->'gaps')>200 or tracking_total<0 or tracking_total>duration or tracking_total<>trunc(tracking_total) then
      raise exception 'invalid tracking bounds' using errcode='22023';
    end if;
    for gap in select * from jsonb_array_elements(tracking->'gaps') loop
      if jsonb_typeof(gap) is distinct from 'object'
        or jsonb_typeof(gap->'start_ms') is distinct from 'number'
        or jsonb_typeof(gap->'end_ms') is distinct from 'number'
        or coalesce(gap->>'reason','') not in ('unreliable','no_result') then
        raise exception 'invalid tracking gap' using errcode='22023';
      end if;
      start_ms := (gap->>'start_ms')::numeric; end_ms := (gap->>'end_ms')::numeric;
      if start_ms<gap_end or end_ms<=start_ms or end_ms>duration or start_ms<>trunc(start_ms) or end_ms<>trunc(end_ms) then
        raise exception 'invalid tracking interval' using errcode='22023';
      end if;
      recorded := recorded+end_ms-start_ms; gap_end := end_ms;
      gaps := gaps || jsonb_build_array(jsonb_build_object('start_ms',start_ms,'end_ms',end_ms,'reason',gap->>'reason'));
    end loop;
    if tracking_total<recorded or (tracking->>'truncated')::boolean<>(tracking_total>recorded)
      or ((tracking->>'truncated')::boolean and jsonb_array_length(gaps)<>200) then
      raise exception 'inconsistent tracking total' using errcode='22023';
    end if;
    tracking := jsonb_build_object('gaps',gaps,'total_ms',tracking_total,'truncated',tracking->'truncated');
  end if;
  return jsonb_build_object('version',1,'status',report->>'status','algorithm','rules-v1','source','device_estimate','stance',report->>'stance','duration_ms',duration,'events',events,'counts',counts,'total_points',total,'mean_quality',case when jsonb_array_length(events)>0 then round(quality_sum::numeric/jsonb_array_length(events)) else null end) || case when tracking is not null then jsonb_build_object('tracking',tracking) else '{}'::jsonb end;
end
$$;
